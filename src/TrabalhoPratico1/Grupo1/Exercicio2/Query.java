...
// Princípio do Exercício 2
    ...
    public static Query parse(String queryToParse) {
        ...
        String text = queryToParse.substring(0, queryToParse.indexOf('?')),
    ...
    // Alínea 8 - POSSÍVEL REFATORIZAÇÃO
    public static Query[] growingQueries(Query[] queries) {
        ArrayList<Query> temp = new ArrayList<>(queries.length);
        for (int i = 0; i < queries.length; i++) {
            ...
            for (int j = 0; j <= temp.size(); j++) {
                if (queries[i].getPoints() >= temp.get(j).getPoints()) {
                    temp.add(j, queries[i]);
                    break;
                }
                if (j == temp.size()) temp.add(queries[i]);
            }
        }
